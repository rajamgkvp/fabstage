<?php
	...
	//CODE TO UPLOAD PICTURE OR IMAGE FILE
		if($_FILES['upload_pic']['name']!="") {
			$target = "admin/portfolio_talent_pic/"; 
			...
			if((($_FILES["upload_pic"]["type"] == "image/gif")
				...
				&& in_array($extension, $allowedExts)) {
					if ($_FILES["upload_pic"]["error"] > 0) {
						...
						$sql1 = q("INSERT INTO fs_portfolio_talent_photo (ex_id,title,large_pic,added_on)values('".$_SESSION['fab_id']."','".$_REQUEST['ptitle']."','".$name."','".date('Y-m-d h:i:s')."')");
						if(!$sql1) {
	...
	if($_FILES['upload_audio']['name']!="") {
		$target = "admin/portfolio_talent_audio/"; 
			...
			|| ($_FILES["upload_audio"]["type"] == "audio/wav"))) {
				...
				if ($_FILES["upload_audio"]["error"] > 0) {
					...
					if (file_exists("images/" . $_FILES["upload_audio"]["name"])) {
						...
						$sql1 = q("INSERT INTO fs_portfolio_talent_audio (ex_id,title,large_pic,added_on)values('".$_SESSION['fab_id']."','".$_REQUEST['atitle']."','".$name."','".date('Y-m-d h:i:s')."')");
						if(!$sql1) {
	...
	//CODE TO UPLOAD RESUME FILE
		if($_FILES['upload_resume']['name']!="") {
			$target = "admin/portfolio_talent_logo/"; 
			...
			if(in_array($extension, $allowedExts)) {
					if ($_FILES["upload_resume"]["error"] > 0) {
						...
						$sql1 = q("INSERT INTO fs_portfolio_talent_logo (ex_id,title,large_pic,added_on)values('".$_SESSION['fab_id']."','".$_REQUEST['rtitle']."','".$resume."','".date('Y-m-d h:i:s')."')");
						if(!$sql1) {
	...
	if($_REQUEST['video']!="") {
		$sql_video = q("INSERT INTO fs_portfolio_talent_video (ex_id,title,large_pic,added_on)values('".$_SESSION['fab_id']."','".$_REQUEST['vtitle']."','".$_REQUEST['video']."','".date('Y-m-d h:i:s')."')");
		if(!$sql_video) {
	   ...
	   <a href="http://fs.sitebysite.us/talent_step5.php" class="text">Fifth Step <span></span></a> 
		...
		<form enctype="multipart/form-data" name="upload_form" action="" method="POST"  autocomplete="off" style="font:normal 13px arial;">
			...
			<table width="100%" border="0" align="left" cellspacing="0" cellpadding="0">
				...
				<tr>
					<td colspan="2" align="center" class="msg" style="font-size: 12px">
						<?
							if($msg_image != "") {
								echo $msg_image;
							}
				...
				<tr>
					<td class="text_heading" align="left">Resume Title </td>
				...
				<tr>
					<td class="text_heading" align="left">Upload Resume</td>
