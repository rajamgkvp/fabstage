...
if($_FILES['pic']['name'] != ""){	//UPLOAD LOGO
	...
	//LARGE PIC
	$uploaddir = 'admin/portfolio_audio/';
	$input_name = 'pic';
	...
	if($file['error'] == UPLOAD_ERR_OK){
		if(isAllowedExtension($file['name'])){
			if(move_uploaded_file($_FILES[$input_name]['tmp_name'], $uploaddir.$uploadfile)){
				...
				$sql = "INSERT INTO fs_portfolio_audio (ex_id, title, large_pic,  added_on) VALUES ('".$id."', '".$_REQUEST['title']."', '".$uploadfile."',  '".date('Y-m-d h:i:s')."')";
...
function delete_pub(id,id1){
	...
	window.location.href='delete_audio_front.php?id='+id+'&id1='+id1;
}
</script>
    ...
    <td colspan="2" height="8" width="100%">
		<table cellpadding="1" cellspacing="1" width="100%">
		   <tr style="background-color: #669966">
			  ...
			  <?
				$sqlg = "Select * from fs_portfolio_audio Where ex_id = '".$id."' order by fld_id DESC";
				$resg = q($sqlg);
				$countg = nr($resg);
				if($countg){
					while($rowg = f($resg)){
