...
$query = "update fs_message set company_status=2 where  (sender_id='".$_SESSION['fab_id']."' OR sender_id='".$_REQUEST['talent_id']."') AND (receiver_id='".$_SESSION['fab_id']."' OR receiver_id='".$_REQUEST['talent_id']."') AND job_id = '".$_REQUEST['job_id']."' "; 
   ...
   if(isset($_REQUEST['submit'])){
		$query = "insert into fs_message(job_id, sender_id,receiver_id,message,time)values('".$_REQUEST['job_id']."','".$_SESSION['fab_id']."','".$_REQUEST['talent_id']."','".$_REQUEST['message']."','".date('Y-m-d h:i:s')."')";
		$insert = q($query);
		if($insert){
                    	...
                    	<tr>
                        	<td class="message_listing">
                            	<ul>
									...
									<li>
                                    <a href="message_company_page.php?job_id=<?=$job_result['job_id']?>&talent_id=<?=$job_result['talent_id']?>" <?if($job_result['job_id']==$_REQUEST['job_id'] AND $job_result['talent_id']==$_REQUEST['talent_id']){?>class="active"<?}?>>
                               ...
                               <?php
									...
									<li>
                                    <a href="message_company_audition_page.php?audition_id=<?=$job_result1['audation_id']?>&talent_id=<?=$job_result1['talent_id']?>">
									<?if($qls_fatch_comp['profile_crop_image']!=''){?>
									<img src="<?=$qls_fatch_comp['profile_crop_image']?>" width="50" height="50" />
									<?}else{?>
                        	...
                        	<td>
							  ...
							  ?>
								...
								<? 
									...
									$message_list_view = q("SELECT * FROM  fs_message WHERE (sender_id='".$_SESSION['fab_id']."' OR sender_id='".$_REQUEST['talent_id']."') AND (receiver_id='".$_SESSION['fab_id']."' OR receiver_id='".$_REQUEST['talent_id']."') AND job_id = '".$_REQUEST['job_id']."' order by fld_id DESC ");
