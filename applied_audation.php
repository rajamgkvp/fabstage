...
<script type="text/javascript">
	function getConfirmation(j_id,t_id){
		...
		if(retVal == true ){
			window.location.href="change_audition_status.php?jobid="+j_id+"&talentid="+t_id;
			return true;
		}else{
			alert("Thank You");
			return false;
					...
					<!-- INCLUDED FILE TO DISPLAY SIDE TAB ON CENTER TAB -->
					 <?include('left_work_room.php');?>
						...
						<table class="center_body" width="100%"  border="0" align="right" cellspacing="1" cellpadding="1" style="margin-top:0px;float:right;">
							...
							<tr>
								<?php
									$job_query = q("SELECT * FROM fs_applied_audation WHERE talent_id='".$_SESSION['fab_id']."' AND status=3");
								...
								<!-- <td width="55%">
									<?php 
									$title_query = q("SELECT * FROM fs_oudition WHERE fld_id='".$job_result['audation_id']."'");
									$title_res = f($title_query);
									?>
									<a href="display_audition_details.php?id=<?=$title_res['fld_id']?>"><?=$title_res['title']?></a>
								</td> -->
							...
							?>
                        <h1><a href="display_audition_details.php?id=<?=$title_res['fld_id']?>"><?=$title_res['title']?></a></h1>
                        <table width="100%" border="0" cellspacing="0" cellpadding="0" style="clear:both;">
                          <tr>
                            ...
                            <?}?>
                            <td valign="top" class="talent_result"><h1><a href="display_audition_details.php?id=<?=$title_res['fld_id']?>"><?=$fatch_company_info['name']?></a></h1>
							...
							<!--------------->
								...
								<div class="button1" style="width:100px;">
										<a href="talent_applied_audition_msg.php?company_id=<?=$job_result['company_id']?>&audition_id=<?=$job_result['audation_id']?>"  title="Send Message" rel="gb_page_center[540, 300]" style="cursor: pointer; line-height:21px; color:#fff;">Send Message</a>
									</div>
								</td>
							</tr>
							<?$count++;}?>
							<?} else {?>
								<tr>
									<td colspan="4"> <a> List not found...</a></td>
								 </tr>
							<?}?>
						</table>
						...
						<table class="center_body" width="100%"  border="0" align="right" cellspacing="1" cellpadding="1" style="float:right;">
							...
							<tr>
								<?php  
									$job_query = q("SELECT * FROM fs_applied_audation WHERE talent_id='".$_SESSION['fab_id']."' AND status=2");
								$job_number1 = nr($job_query);
								if($job_number1!=0) {
									$count=1;
									while($job_result = f($job_query)) {
								...
								<!-- <td width="65%">
									...
									$title_res = f($title_query);
									?>
									<a href="display_audition_details.php?id=<?=$title_res['fld_id']?>">
									<?=$title_res['title']?></a>
								</td>
 -->
                        ...
                        <table width="100%" border="0" cellspacing="0" cellpadding="0" style="clear:both;">
                          <tr>
							...
							<!--------------->
								...
								<td width="30%">
									<a>
										<?if($job_result['status']==2) {?>
											<input type="button" name="ibtn" class = "button" value="Intrested ?" onclick="getConfirmation(<?=$job_result['audation_id']?>,<?=$job_result['talent_id']?>);">
										<?}?>
									</a>
								</td>
							</tr>
						...
						<table class="center_body" width="100%"  border="0" align="right" cellspacing="1" cellpadding="1" style="float:right;">
							...
							<tr>
                               ...
                               $limit2 = "ORDER BY fld_id DESC LIMIT ".$off.",".$value." ";
									...
									$job_query = q("SELECT * FROM fs_applied_audation WHERE talent_id='".$_SESSION['fab_id']."' AND status=1 ".$limit2."");
									$job_number1 = nr($job_query);
									if($job_number1!=0) {
									$count=1;
									while($job_result = f($job_query)) {
                          ...
                          <tr>
                            ...
                            <tr >
                                  <td colspan="3" align="center" height="10px" style="color: #B00000">
                                            <?
                                             $sqlag = "SELECT * FROM fs_applied_audation WHERE talent_id='".$_SESSION['fab_id']."' AND status=1";
                                       
                                                $rescs = q($sqlag);
                                                 $countcs = nr($rescs);
                                                if($countcs >5){
                                                    doPages(5, 'applied_audation.php', '', $countcs); 
                                                }
                                            ?>
                                   </td>
                           </tr>
