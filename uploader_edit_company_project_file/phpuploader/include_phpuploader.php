...
function PhpUploader_MoveUploadedFile($_file,$_line,$src,$dst)
{
	...
	$re=move_uploaded_file(iconv($PhpUploader_InternalEncoding,$PhpUploader_FSEncoding,$src),iconv($PhpUploader_InternalEncoding,$PhpUploader_FSEncoding,$dst));
	   	  $pic_name = substr($dst,65,-5);
	

      $query = "insert into fs_company_project_portfolio (company_id,project_id,portfolio_data,added_on) values ('".$_SESSION['fab_id']."','".$_SESSION['project_id']."','".$pic_name."','".date('Y-m-d h:m:s')."') ";


	echo $run = mysql_query($query);
