...
if($_FILES['pic']['name'] != ""){	//UPLOAD LOGO
	...
	//LARGE PIC
	$uploaddir = 'portfolio_talent_audio/';
	$input_name = 'pic';
	...
	if($file['error'] == UPLOAD_ERR_OK){
		if(isAllowedExtension($file['name'])){
			if(move_uploaded_file($_FILES[$input_name]['tmp_name'], $uploaddir.$uploadfile)){
				...
				$sql = "INSERT INTO fs_portfolio_talent_audio (ex_id, title, large_pic,  added_on) VALUES ('".$_REQUEST['id']."', '".$_REQUEST['title']."', '".$uploadfile."',  '".date('Y-m-d h:i:s')."')";
...
function delete_pub(id,id1){
	...
	window.location.href='delete_talent_audio.php?id='+id+'&id1='+id1;
}
</script>
...
<tr>
  ...
  <tr>
    <td colspan="2" align="left" class="contentheading">Add Portfolio Audio :   </td>
  </tr>
  <tr>
		...
		<table cellpadding="2" cellspacing="2">
		   <tr>
			  ...
			  <tr>
			   ...
			   <tr>
					<td width="150px"><span class="style1">*</span>&nbsp;<span>Browse  Audio</span> </td>
			  ...
			  <tr>
			   ...
			   <tr>
				  <td>&nbsp;</td>
					<td>
					  <input type="submit" value="Add Audio" name="Submit">
				   </td>
				</tr>
		</table>
		<form>
  ...
  <tr>
   ...
   <tr>
    <td colspan="2" align="left" class="contentheading">Portfolio Audio List</td>
  </tr>
   <tr>
    <td colspan="2" height="10" align="center"></td>
  </tr>
    ...
    <td colspan="2" height="8" width="100%">
		<table cellpadding="1" cellspacing="1" width="100%">
		   <tr style="background-color: #003162">
			  ...
			  <?
				$sqlg = "Select * from fs_portfolio_talent_audio Where ex_id = '".$_REQUEST['id']."' order by fld_id DESC";
				$resg = q($sqlg);
				$countg = nr($resg);
				if($countg){
					while($rowg = f($resg)){
			  ...
			  <tr>
					...
					<td height="25px"><img src="portfolio_talent_audio/<?=$rowg['large_pic']?>" width="100px"></td>
