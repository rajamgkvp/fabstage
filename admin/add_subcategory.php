...
if(isset($_REQUEST['title']) && $_REQUEST['title'] != ''){
	$sql_duplicate = "SELECT * FROM fs_sub_category WHERE title = '".$_REQUEST['title']."'";
	$res_duplicate = q($sql_duplicate);
	$count = nr($res_duplicate);
	if($count == 0){
		$sql_admin = "INSERT INTO fs_sub_category (category_id, sub_category, description, added_on) VALUES('".$_REQUEST['ntype']."',  '".$_REQUEST['title']."', '".$_REQUEST['desc1']."','".date('Y-m-d h:m:s')."')";
		$res_admin = q($sql_admin);
		if($res_admin){
			$msg = '- Sub Category Added Successfully.';
		}
	}else{
		$msg = '- Duplicate Sub Category Title Not Allowed.';
	}
}
?>
  ...
  <tr>
    ...
    <td>
		...
		<select name="ntype" id="ntype" style="font-family: verdana; font-size: 12px; color: #005EBB">
			...
			<?$sql = "SELECT * FROM fs_category order by category_name";
	           $res = q($sql);	 
			   while($fatch = f($res)){
			   ?>
