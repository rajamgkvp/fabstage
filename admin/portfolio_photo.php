	...
	if($file['error'] == UPLOAD_ERR_OK){
		if(isAllowedExtension($file['name'])){
			if(move_uploaded_file($_FILES[$input_name]['tmp_name'], $uploaddir.$uploadfile)){
				...
				$sql = "INSERT INTO fs_portfolio_photo (ex_id, title, large_pic,  added_on) VALUES ('".$_REQUEST['id']."', '".$_REQUEST['title']."', '".$uploadfile."',  '".date('Y-m-d h:i:s')."')";
...
function delete_pub(id,id1){
	...
	window.location.href='delete_photo.php?id='+id+'&id1='+id1;
}
</script>
    ...
    <td colspan="2" height="8" width="100%">
		<table cellpadding="1" cellspacing="1" width="100%">
		   <tr style="background-color: #003162">
			  ...
			  <?
				$sqlg = "Select * from fs_portfolio_photo Where ex_id = '".$_REQUEST['id']."' order by fld_id DESC";
				$resg = q($sqlg);
				$countg = nr($resg);
				if($countg){
					while($rowg = f($resg)){
			  ...
			  <tr>
					...
					<td height="25px"><img src="portfolio_pic/<?=$rowg['large_pic']?>" width="100px"></td>
