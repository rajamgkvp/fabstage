...
if(isset($_REQUEST['title']) && $_REQUEST['title'] !=''){
	...
	$sql1 = "SELECT * FROM fs_sub_category WHERE title = '".$_REQUEST['title']."' AND fld_id != ".$id."";
	$res1 = q($sql1);
	$count = nr($res1);
	if($count == 0){

		//UPDATE THE USER TABLE
		$sql = "UPDATE fs_sub_category SET category_id = '".$_REQUEST['ntype']."', sub_category = '".$_REQUEST['title']."', description = '".$_REQUEST['desc1']."'  WHERE fld_id = ".$id."";
		$res = q($sql);
...
//POPULATE 
$sql_pop = "SELECT * FROM fs_sub_category WHERE fld_id = ".$id."";
$res_pop = q($sql_pop);
$row = f($res_pop);
?>
