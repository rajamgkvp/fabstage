...
<script>
	function Inc() {
		<?php
			$sql_apply = "select * from fs_applied_audation where talent_id = '".$_SESSION['fab_id']."' AND audation_id = '".$_REQUEST['id']."'";
			$run_apply = q($sql_apply);
			$num_apply = nr($run_apply);
			if($num_apply==0) {
				$job_insert = "insert into fs_applied_audation(talent_id,audation_id,company_id,added_on)values('".$_SESSION['fab_id']."','".$_REQUEST['id']."','".$sql_fatch['company_id']."','".date('Y-m-d h:m:s')."')";
				$job_run = q($job_insert);
				if($job_run) {
					$msg_job = "Submitted Successfully";

					//window.location.href = "view_audation_details.php?id=<?=$_REQUEST['id']?>";

				}
			} else {
				$msg_job = "You has been allready applied for this Audation ";
			}
		?>
		alert("<?=$msg_job?>");
	}
		...
		<div>
			<div class="main_body_area">
				...
				<div class="center_body" style="width:800px;">
					...
					<tr>
						<td colspan="2" align="left" style="color:#fff; font-size:16px; font-weight:normal; background:#6BC0F7; height:30px; text-indent:10px; border:2px solid #fff;">
						<img src="images/details.gif" style="vertical-align:text-top; padding-right:5px;" /> Audation Roles  <?if($number>1){?> [<?=$i?>]<?}?></td>
					</tr>
					<tr>
						<td width="30%">
