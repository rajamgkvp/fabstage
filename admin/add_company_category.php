...
if(isset($_REQUEST['skill']) && $_REQUEST['skill'] != ''){
	$sql_duplicate = "SELECT * FROM fs_company_category WHERE category_name = '".$_REQUEST['skill']."'";
	$res_duplicate = q($sql_duplicate);
	$count = nr($res_duplicate);
	if($count == 0){
		$sql_admin = "INSERT INTO fs_company_category (category_name,  fld_addedon) VALUES('".$_REQUEST['skill']."', '".date('Y-m-d h:m:s')."')";
		$res_admin = q($sql_admin);
		if($res_admin){
			$msg = '- Added Successfully.';
		}
	}else{
		$msg = '- Duplicate Company Category Not Allowed.';
	}
...
function validate(){
	...
	if(document.upload_form.skill.value == ""){
		 errText += "- Please Enter Category name.\n";
		 alert(errText);
		 errorflag = true;
...
<tr>
  ...
  <tr>
    <td colspan="2" width="100%" align="center" class="contentheading">Add Comapny Category </td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
