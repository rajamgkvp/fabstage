...
<p><h3>Add Skill <a href="add_skill.php" title="Add Skill" rel="gb_page_center[540, 520]" style="cursor: pointer;">Click Here</a></h3></p>
...
$comm=q("SELECT * FROM fs_skill ORDER BY `fld_id` DESC");
if(nr($comm) > 0)
{
$paging=new paging(25,5);
$paging->query("SELECT * FROM fs_skill ORDER BY `fld_id` DESC");
$page=$paging->print_info();
	$x=1;
	while($rec_mem=$paging->result_assoc()){	
	 	$color = $x%2==0?'#E9F3F3':'#FFFFEA';
		$ntype = $rec_mem['fld_id'];
		$psd = $rec_mem['skill_name'];
