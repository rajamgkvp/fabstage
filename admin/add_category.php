...
if(isset($_REQUEST['skill']) && $_REQUEST['skill'] != ''){
	$sql_duplicate = "SELECT * FROM fs_category WHERE category_name = '".$_REQUEST['skill']."'";
	$res_duplicate = q($sql_duplicate);
	$count = nr($res_duplicate);
	if($count == 0){
		$sql_admin = "INSERT INTO fs_category (category_name,  fld_addedon) VALUES('".$_REQUEST['skill']."', '".date('Y-m-d h:m:s')."')";
		$res_admin = q($sql_admin);
		if($res_admin){
			$msg = '- Added Successfully.';
		}
	}else{
		$msg = '- Duplicate Category Not Allowed.';
	}
...
<tr>
  ...
  <tr>
    <td colspan="2" width="100%" align="center" class="contentheading">Add Category </td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
