...
function delete_news(id){
	...
	window.location.href='delete_company_sub_category.php?id='+id;
}						   
...
<p><h3>Add Company Sub Category <a href="add_company_subcategory.php" title="Add Company Sub Category" rel="gb_page_center[540, 520]" style="cursor: pointer;">Click Here</a></h3></p>
...
<tr><h2>Company Sub Category List</h2>
...
$comm=q("SELECT * FROM fs_company_sub_category ORDER BY `fld_id` DESC");
if(nr($comm) > 0)
{
$paging=new paging(25,5);
$paging->query("SELECT * FROM fs_company_sub_category ORDER BY `fld_id` DESC");
$page=$paging->print_info();
	$x=1;
	while($rec_mem=$paging->result_assoc()){	
	 	$color = $x%2==0?'#E9F3F3':'#FFFFEA';

		$ntype = $rec_mem['category_id'];
		$sql_duplicate = "SELECT * FROM fs_company_category WHERE fld_id = '".$ntype."'";
	    $res_duplicate = q($sql_duplicate);
		$fatch_category = f($res_duplicate);
		$category_name = $fatch_category['category_name'];
		
		$title = $rec_mem['sub_category'];

		$pro = $rec_mem['description'];
	...
	<tr align="right" bgcolor="<?=$color?>">
		<td><?=$category_name?> </td>
		<td><?=$title?></td>
		<td><?=substr($pro,0,100)?>...</td>
		<td><?=$added_on?></td>

		<td><a href="company_sub_category_details.php?id=<?=$rec_mem['fld_id']?>" title="Company Sub Category Details" rel="gb_page_center[540, 520]" style="cursor: pointer;"><U>View Details</U></a></td>

		<td><a href="edit_company_sub_category.php?id=<?=$rec_mem['fld_id']?>" title="Edit Company Sub Category" rel="gb_page_center[800, 400]" style="cursor: pointer;"><img src="admin_img/edit.png" border="0"></a></td>
