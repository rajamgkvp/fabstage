...
if($_REQUEST['name']!= ""){	//UPLOAD LOGO
				...
				$sql = "INSERT INTO fs_client_contact (client_id, name,address,email,phone,url,  added_on) VALUES ('".$_REQUEST['id']."', '".$_REQUEST['name']."','".$_REQUEST['address']."','".$_REQUEST['email']."', '".$_REQUEST['phone']."','".$_REQUEST['url']."',  '".date('Y-m-d h:i:s')."')";
...
function edit_news(id){
	...
	window.open('edit_client_contact.php?id='+id,'',winparam);
}
</script>
...
function delete_pub(id){
	...
	window.location.href='delete_company_contact.php?id='+id+'&pro_id=<?=$_REQUEST['id']?>';
	}
}
</script>
    ...
    <td colspan="2" height="8" width="100%">
		<table cellpadding="1" cellspacing="1" width="100%">
		   <tr style="background-color: #003162">
			  ...
			  <?
				$sqlg = "Select * from fs_client_contact Where client_id = '".$_REQUEST['id']."' order by fld_id DESC";
				$resg = q($sqlg);
				$countg = nr($resg);
				if($countg){
					while($rowg = f($resg)){
