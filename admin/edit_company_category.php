...
if(isset($_REQUEST['skill']) && $_REQUEST['skill'] !=''){
	...
	$sql1 = "SELECT * FROM fs_company_category WHERE category_name = '".$_REQUEST['skill']."' AND fld_id != ".$id."";
	$res1 = q($sql1);
	$count = nr($res1);
	if($count == 0){

		//UPDATE THE USER TABLE
		$sql = "UPDATE fs_company_category SET category_name = '".$_REQUEST['skill']."' WHERE fld_id = ".$id."";
		$res = q($sql);
...
//POPULATE 
$sql_pop = "SELECT * FROM fs_company_category WHERE fld_id = ".$id."";
$res_pop = q($sql_pop);
$row = f($res_pop);
?>
...
<form name="upload_form" action="" method="POST" onsubmit="return validate();">
  ...
  <table width="60%" border="0" align="center">
    ...
    <tr>
      <td colspan="4"><div align="left" class="contentheading">Update Company Category</div></td>
    </tr>
