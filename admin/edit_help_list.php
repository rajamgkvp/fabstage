<?php
	...
	if(isset($_REQUEST['question']) && $_REQUEST['question'] != ""){
		...
		if($query_done){
			$msg = '- Congratulation! Updated Successfully.';
		}else{
			$msg = '- Sorry! Not Updated Successsfully.';
		}
	}
...
function validate(){
	...
	if(document.upload_form.question.value == ""){
		 errText += "- Please Select Question\n";
		 alert(errText);
		 errorflag = true;
		...
		<form name="upload_form" action="" method="POST" onsubmit="return validate();">
			<table width="300"  border="0" align="center" cellspacing="1" cellpadding="1">
					...
					<td>
						<select name="title" id="title" onchange="getCityList(this)">
							...
							<?php
							 ...
							 ?>
								<option value="<?=$result['fld_id']?>" <?if($result['fld_id']==$get_res['Theory_title']){?> selected<?}?>><?=$result['title']?></option>
							<?}?>
						</select>
					</td>
				</tr>
                 ...
                 <tr>
				    ...
				    <td>
					   <select name="title1" id="title1">
						   ...
						   <?php
						   $mysql="SELECT * FROM add_topic where theoryid = '".$get_res['Theory_title']."'";
						   $res_topic = q($mysql);
						   while($resul = f($res_topic)){
						   ?>
						   
						      <option value="<?=$resul['fld_id']?>" <?if($resul['fld_id']==$get_res['title_id']){?>selected<?}?>><?=$resul['title']?></option>
				...
				<tr>
					...
					<td>
						<textarea class="textarea" name="question" id="question" wrap="physical" rows="8" cols="40"><?=$get_res['question']?></textarea>
					</td>
				</tr>
				<tr>
					<td>&nbsp;</td>
				...
				<tr>
					...
					<td>
						<textarea class="textarea" name="answer" id="answer" wrap="physical" rows="8" cols="40"><?=$get_res['answer']?></textarea>
					</td>
				</tr>
				<tr>
					<td>&nbsp;</td>
				...
				<tr>
					<td>&nbsp;</td>
					<td>
					  <input type="submit" value="Update Question Answer" name="Submit">
				   </td>
				</tr>
				<tr>
					<td height="40px">&nbsp;</td>
