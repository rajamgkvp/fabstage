...
$id = $_REQUEST['id']; 
  if($_REQUEST['name']!="") {
	  ...
	  $sql1 = "UPDATE fs_client_contact SET name = '".$_REQUEST['name']."', address = '".$_REQUEST['address']."', email = '".$_REQUEST['email']."', phone ='".$_REQUEST['phone']."',url ='".$_REQUEST['url']."',updated_on='".date('Y-m-d h:i:s')."'  WHERE fld_id = '".$id."'";
...
//POPULATE 
$sql_pop = "SELECT * FROM fs_client_contact WHERE fld_id = '".$id."'";
$res_pop = q($sql_pop);
$row = f($res_pop);
?>
			 ...
			 <tr>
			  ...
			  <tr>
			   ...
			   <tr>
				  <td>&nbsp;</td>
					<td>
					  <input type="submit" value="Edit Contact" name="Submit">
				   </td>
				</tr>
		</table>
