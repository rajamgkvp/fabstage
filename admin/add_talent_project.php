...
if($_FILES['pic']['name'] != ""){	//UPLOAD LOGO
	...
	//LARGE PIC
	$uploaddir = 'portfolio_talent_project/';
	$input_name = 'pic';
	...
	if($file['error'] == UPLOAD_ERR_OK){
		if(isAllowedExtension($file['name'])){
			...
			$sql1 = "INSERT INTO fs_portfolio_talent_project (ex_id,pro_title, from1,to1,des, doc,  added_on) VALUES ('".$_REQUEST['id']."','".$_REQUEST['pro_title']."', '".$_REQUEST['title']."','".$_REQUEST['title1']."','".$_REQUEST['des']."', '".$uploadfile."',  '".date('Y-m-d h:i:s')."')";
...
function delete_pub(id){
	...
	window.location.href='delete_talent_project.php?id='+id+'&pro_id=<?=$_REQUEST['id']?>';
}
    ...
    <td colspan="2" height="8" width="100%">
		<table cellpadding="1" cellspacing="1" width="100%">
		   <tr style="background-color: #003162">
			  ...
			  <?
				$sqlg = "Select * from fs_portfolio_talent_project Where ex_id = '".$_REQUEST['id']."' order by fld_id DESC";
				$resg = q($sqlg);
				$countg = nr($resg);
				if($countg){
					while($rowg = f($resg)){
