...

{
   ...
   if ( $_SESSION['security_code'] == $_REQUEST['captcha']) 
	
	{
					...
					$sqls = "INSERT INTO fs_feedback (name,email,contact,feedback,rating,added_on)VALUES ('".$_REQUEST['nam']."','".$_REQUEST['email']."','".$_REQUEST['cont']."','".$_REQUEST['msg']."','".$_REQUEST['rating']."','".date('Y-m-d h:m:s')."') ";
					$ress = q($sqls);
	    ...
	    $msg1 = '<span style="font-size:12px;font-family:arial;color:#006633"><b>Your Feedback Successully Send.</b></span>';
...
<script language="javascript" type="text/javascript">
 function validatequiry(){
	...
	if(document.enquiryform.msg.value == ""){
		 errText = "Please Enter Your Feedback.";
		 alert(errText);
			...
			<div>
				<div class="main_body_area">
					...
					<div class="center_body" style="width:800px;">
					 ...
					 $select_testi = q("select * from fs_feedback where status1=2 ".$limit);
					          $count_d = nr($select_testi);
							  if($count_d!=0){
							   while($fatch_testi = f($select_testi)){

								   if($fatch_testi['rating']=="Poor")
								   {
								      $img = "images/star1.png";
								   }else if($fatch_testi['rating']=="Average"){
								      $img = "images/star2.png";
								   }else if($fatch_testi['rating']=="Good"){
								      $img = "images/star3.png";
								   }else if($fatch_testi['rating']=="Very Good"){
							...
							?>
							<tr>
						<td class="colorrr" style="color:#cc0000;font-size:14px;font-weight:bold;"><?=ucwords($fatch_testi['name'])?> <img src="<?=$img?>" >
							...
							<tr>
								<td style="border-bottom:1px #999 dotted;padding-bottom:5px;" colspan="2"><?=$fatch_testi['feedback']?>
                ...
                <tr>
                    <td height="10px" style="color: #B00000">
                                            <?
                                             $sqlag = "SELECT * FROM fs_feedback";
                                       
                                                $rescs = q($sqlag);
                                                 $countcs = nr($rescs);
                                                if($countcs >10){
                                                    doPages(10, 'feedback.php', '', $countcs); 
                                                }
                                            ?>
                    </td>
                </tr>
                ...
                <tr bgcolor="#CCCCFF">
                      <td style="font-family:arial;padding:5px 5px 5px 5px"><span style="font-size:14px;font-family:arial"><b>Any Feedback not found....</b></span></td>
                </tr>
                                ...
                                <td class="enquiry_bg" valign="top">
									<form name="enquiryform" action="" onsubmit="return validatequiry();" method="POST" enctype="">
                                    	...
                                    	<table width="500" border="0" cellspacing="6" cellpadding="0">
                                          <tr><td class="heading" colspan="2" style="font-size:16px;font-weight:bold;font-family:arial;color:#3B8EBA">Submit Your Feedback</td></tr>
										  <tr>
                                          ...
                                          <tr>	
                                          	<td class="form_text" style="font-size:14px;font-family:arial">Feedback:</td>
                                            <td><textarea cols="60" rows="10" name="msg" id="msg" class="form_textarea"></textarea></td>
                                          </tr>


                                          <tr>
										  <td class="form_text" style="font-size:14px;font-family:arial">Rating:</td>
										  <td>
										...
										<tr>
                                          	<td></td>
                                            <td><input type="image" src="images/submit.PNG" name="" width="100"  style="border:none;"  /></td>
                                          </tr>
                                        </table>
