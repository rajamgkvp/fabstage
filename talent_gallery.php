	   ...
	   if($_REQUEST['video_link']!=''){
	     ...
	     $video = explode(",",$video_link);
			foreach ($video as &$link) {
               $Insert = "insert into fs_talent_portfolio(talent_id,skill,video,added_on)values('".$id."','".$check_val."','".$link."','".date('Y-m-d h:m:s')."') ";
	 ...
	 <tr>
	  <td valign="top">
	       <div style="float:left; width:170px; clear:both;">
				<?include('include_talent_gallery_skill.php');?>
			</div>
	  </td>
...
<!------upload Portfolio-------->
	...
	<form enctype="multipart/form-data" onSubmit="return validate5();" name="upload_form" id="upload_form" action="" method="POST"  autocomplete="off" style="font:normal 13px arial;">
							      ...
							      <?php
								       ...
								       $sql = "select * from fs_talent where member_id = '".$id."' ";
								       $res = q($sql);	
								             while($fatch = f($res)){ 
...
?>
 ...
 <div style="width:100%; float:left; margin-bottom:25px;">
	   ...
	   <ul class="thumbnail-tabs">
		   ...
		   <?
				if($_REQUEST['skill']==''){
					   ...
					   $query_gallery = "select * from fs_talent_portfolio where talent_id = '".$id."' AND (portfolio_data LIKE '%".$jpg."%' OR portfolio_data LIKE '%".$png."%' OR portfolio_data LIKE '%".$gif."%')";
					   $run = q($query_gallery);
	...
	<?}else{
					   ...
					   $query_gallery = "select * from fs_talent_portfolio where talent_id = '".$id."' AND (portfolio_data LIKE '%".$jpg."%' OR portfolio_data LIKE '%".$png."%' OR portfolio_data LIKE '%".$gif."%') AND skill LIKE '%".$_REQUEST['skill']."%'";
					   $run = q($query_gallery);
					   while($run_gallery = f($run)){
...
<!------portpolio audio start------------>	 
    ...
    <?
					   ...
					   if($_REQUEST['skill']==''){
					   $query_gallery = "select * from fs_talent_portfolio where talent_id = '".$id."' AND (portfolio_data LIKE '%".$mp3."%' OR portfolio_data LIKE '%".$wav."%')";
                       }else{
					    $query_gallery = "select * from fs_talent_portfolio where talent_id = '".$id."' AND (portfolio_data LIKE '%".$mp3."%' OR portfolio_data LIKE '%".$wav."%')AND skill LIKE '%".$_REQUEST['skill']."%'";
...
?>	
		...
		<?
						...
						$i = 1;
						 if($_REQUEST['skill']==''){
						$query_gallery = "select * from fs_talent_portfolio where talent_id = '".$id."' AND video != ''";
						 }else{
						 
						 $query_gallery = "select * from fs_talent_portfolio where talent_id = '".$id."' AND skill LIKE '%".$_REQUEST['skill']."%'  AND video != ''";
