			...
			<div>
				<div class="main_body_area">
					...
					<!-- INCLUDED FILE TO DISPLAY SIDE TAB ON CENTER TAB -->
					 <?include('advance_search_audition.php');?>
                 ...
                 /////////////////////////
                  ...
                  ////////////////////////////////
                    ...
                    if($ptr!=''){
				            $query_job = "select * from fs_oudition where ".$ptr." order by interview_city ASC";	 
						}else{
						    $query_job = "select * from fs_oudition  order by interview_city ASC";	
						}
				 ...
				 $job_run = q($query_job);
                         ...
                         $query_job1 = "select * from fs_oudition_details  where ".$str." AND oudition_id IN (".$f_id.")   ";
					   }else{
							    
								  $query_job1 = "select * from fs_oudition_details  where  oudition_id IN (".$f_id.")   ";
                          ...
                          if($_REQUEST['page'] == '' || $_REQUEST['page'] == 1){
                            ...
                            $limit2 = "ORDER BY fld_id DESC LIMIT ".$off.",".$value." ";
                              ...
                              $member_fiend = "select * from fs_oudition where fld_id IN (".$f_id.")  ".$limit2." ";

					          
							  
							  
							 // foreach ($array as &$value2) {
							  //$member_fiend = "select * from fs_oudition where fld_id = '".$value2."' ";
                    ...
                    <!------------------>
                        ...
                        <h1><a href="view_audation_details.php?id=<?=$fatchhh['fld_id']?>"><?=$fatchhh['title']?></a></h1>
                        	<table width="100%" border="0" cellspacing="0" cellpadding="0" style="clear:both;">
  <tr>
    <td valign="top" width="70"><img src="<?=$select_company_name['profile_crop_image']?>" width="60" height="60" /></td>
                            <td valign="top" class="talent_result"><h1><a href="view_audation_details.php?id=<?=$fatchhh['fld_id']?>"><?=$select_company_name['name']?></a></h1>
                           	  <p><strong>Location:</strong> <?=$fatchhh['interview_city']?>,<?=$fatchhh['interview_state']?>,<?=$fatchhh['interview_country']?></p>
                              <p><strong>Skills:</strong> <?=str_replace(',,',"'",substr($fatchhh['main_skill'],1,50))?></p></td>
                            <td width="100" style="font-size:11px; text-align:center;" align="center">
                            <a href="view_audation_details.php?id=<?=$fatchhh['fld_id']?>"><input class="search_btn" name="" type="button" value="Apply" /></a><br />
							<em><strong>Added on:</strong><br />
 <?=$fatchhh['added_on']?></em></td>
  </tr>
</table>
						...
						?>
							...
							<tr >
                                  <td colspan="3" align="center" height="10px" style="color: #B00000">
                                            <?
                                             $sqlag = "select * from fs_oudition where fld_id IN (".$f_id.") ";
                                       
                                                $rescs = q($sqlag);
                                                 $countcs = nr($rescs);
                                                if($countcs >8){
                                                    doPages(8, 'audition_search_result.php', 'cat='.$_REQUEST['cat'].'&category='.$_REQUEST['category'].'&location='.$_REQUEST['location'].'&to_experience='.$_REQUEST['to_experience'].'&from_experience='.$_REQUEST['from_experience'].'&language='.$_REQUEST['language'].'&wag_to='.$_REQUEST['wag_to'].'&wag_from='.$_REQUEST['wag_from'].'&assosiation_name='.$_REQUEST['assosiation_name'].'&nationality='.$_REQUEST['nationality'].'&to_height='.$_REQUEST['to_height'].'&from_height='.$_REQUEST['from_height'].'&weight='.$_REQUEST['weight'].'&ethenicity='.$_REQUEST['ethenicity'].'&eye='.$_REQUEST['eye'].'&hair='.$_REQUEST['hair'].'&body='.$_REQUEST['body'].'&skin='.$_REQUEST['skin'].'&shoes='.$_REQUEST['shoes'].'&dress='.$_REQUEST['dress'].'&look='.$_REQUEST['look'].'', $countcs); 
                                                }
                                            ?>
                                   </td>
                           </tr>	
