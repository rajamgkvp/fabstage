...
myflowers.init()
		  ...
		  <?
				...
				$limit = "LIMIT ".$off.",".$value." ";
                 ...
                 $sql="SELECT * FROM fs_talent where main_skill !='' " .$limit;
	             $res=q($sql);
	             $count=nr($res);
	             if($count>0)
	              {
                ...
                $select_mamber = "select * from fs_mamber where fld_id = '".$fatch_talent['member_id']."'";
				$run_mamber = q($select_mamber);
				$fatch_mamber = f($run_mamber);

				$skill = str_replace(",,",",",$fatch_talent['main_skill']);
				$skill = substr($skill,1,-1);

				$language = substr($fatch_talent['language'],1);
                $language = str_replace(",",", ",$language);
		  ?>
		  
		  
		  <div class="job_content">
                	<h1><b><?=ucwords($fatch_mamber['name'])?></b> [<?=$skill?>]</h1>
                    ...
                    <div class="job_details">
						   <table> 
									...
									<tr><td><h5>Work Location: <span><?if($fatch_talent['work_area']!=''){echo ucwords($fatch_talent['work_area']);}else{?>Not Available <?}?></span></h5><tr><td>

									

									<tr><td><h5>Experience: <span><?if($fatch_talent['experience']!=''){echo ucwords($fatch_talent['experience']);}else{?>Not Available <?}?></span></h5><tr><td>

									

									<tr><td><h5>Language Known: <span><?if($fatch_talent['language']!=''){echo ucwords($language);}else{?>Not Available <?}?></span></h5><tr><td>
	  ...
	  <?
		...
		<td colspan="2" height="10px" style="color: #B00000">
								<?
								 $sqlag = "SELECT * FROM fs_talent where main_skill!=''";
						   
									$rescs = q($sqlag);
									 $countcs = nr($rescs);
									if($countcs >5){
										doPages(5, 'find_talent.php', '', $countcs); 
									}
								?>
		</td>
	</tr>
	...
	else
	{
		?>
		<tr bgcolor="#CCCCFF">
		  <td colspan="2" style="font-family:arial;padding:5px 5px 5px 5px"><span style="font-size:14px;font-family:arial"><b>Any Talent not found.... </b></span></td>
	    </tr>
		<?
	}
	?>
