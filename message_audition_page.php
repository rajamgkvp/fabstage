...
include('check_session.php');	
 ...
 $query = "update fs_audition_msg set talent_status=2 where (sender_id='".$_SESSION['fab_id']."' OR sender_id='".$_REQUEST['company_id']."') AND (receiver_id='".$_SESSION['fab_id']."' OR receiver_id='".$_REQUEST['company_id']."') AND audition_id = '".$_REQUEST['audition_id']."' "; 
$query_run = q($query);

if(isset($_REQUEST['submit'])){
		$query = "insert into fs_audition_msg(audition_id, sender_id,receiver_id,message,send_time)values('".$_REQUEST['audition_id']."',
		'".$_SESSION['fab_id']."','".$_REQUEST['company_id']."','".$_REQUEST['message']."',
		'".date('Y-m-d h:i:s')."')";
		$insert = q($query);
		if($insert){
                    	...
                    	<tr>
                        	<td class="message_listing">
                            	<ul>
									...
									<li>
                                    <a href="message_page.php?job_id=<?=$job_result['job_id']?>&company_id=<?=$job_result['company_id']?>">
                               ...
                               <?php
									...
									<li>
                                    <a <?if($job_result1['audation_id']==$_REQUEST['audition_id'] AND $job_result1['company_id']==$_REQUEST['company_id']){?>class="active"<?}?> href="message_audition_page.php?audition_id=<?=$job_result1['audation_id']?>&company_id=<?=$job_result1['company_id']?>">
									<?if($qls_fatch_comp['profile_crop_image']!=''){?>
									<img src="<?=$qls_fatch_comp['profile_crop_image']?>" width="50" height="50" />
									<?}else{?>
							  ...
							  ?>
								...
								<? 
                                   ...
                                   $message_list_view = q("SELECT * FROM fs_audition_msg WHERE
									(sender_id='".$_SESSION['fab_id']."' OR sender_id='".$_REQUEST['company_id']."') AND (receiver_id='".$_SESSION['fab_id']."' OR receiver_id='".$_REQUEST['company_id']."') AND audition_id='".$_REQUEST['audition_id']."' order by fld_id DESC");
