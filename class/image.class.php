...
class Zubrag_image {
  ...
  function GenerateThumbFileSmall($from_name, $to_name) {
    ...
    if ($orig_x > $this->max_x1 or $orig_y > $this->max_y1) {
    ...
    if ($this->image_type == 1) {
      ...
      $ni = imagecreate($this->max_x1, $this->max_y1);
    }
    else {
      // Create a new true color image
      $ni = ImageCreateTrueColor($this->max_x1,$this->max_y1);
    }
 
    // Fill image with white background (255,255,255)
    $white = imagecolorallocate($ni, 255, 255, 255);
    imagefilledrectangle( $ni, 0, 0, $this->max_x1, $this->max_y1, $white);
    ...
    imagecopyresampled(
      ...
      $this->max_x1, $this->max_y1,       // dstW, dstH
  ...
  // generate thumb from image and save it
  function GenerateThumbFileMed($from_name, $to_name) {
    ...
    if ($orig_x > $this->max_x2 or $orig_y > $this->max_y2) {
 
      // resize
      $per_x = $orig_x / $this->max_x2;
      $per_y = $orig_y / $this->max_y2;
      if ($per_y > $per_x) {
        $this->max_x2 = 200;
      }
      else {
    ...
    if ($this->image_type == 1) {
      // should use this function for gifs (gifs are palette images)
      $ni = imagecreate($this->max_x2, $this->max_y2);
    }
    else {
      // Create a new true color image
      $ni = ImageCreateTrueColor($this->max_x2,$this->max_y2);
    }
 
    // Fill image with white background (255,255,255)
    $white = imagecolorallocate($ni, 255, 255, 255);
    imagefilledrectangle( $ni, 0, 0, $this->max_x2, $this->max_y2, $white);
    // Create a new image from source file
    $im = $this->ImageCreateFromType($orig_img_type,$from_name);
    // Copy the palette from one image to another
    imagepalettecopy($ni,$im);
    ...
    imagecopyresampled(
      $ni, $im,             // destination, source
      0, 0, 0, 0,           // dstX, dstY, srcX, srcY
      $this->max_x2, $this->max_y2,       // dstW, dstH
      $orig_x, $orig_y);    // srcW, srcH
