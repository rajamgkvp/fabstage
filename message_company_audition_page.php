...
include('check_session.php');	
 ...
 $query = "update fs_audition_msg set company_status=2 where (sender_id='".$_SESSION['fab_id']."' OR sender_id='".$_REQUEST['talent_id']."') AND (receiver_id='".$_SESSION['fab_id']."' OR receiver_id='".$_REQUEST['talent_id']."') AND audition_id = '".$_REQUEST['audition_id']."' "; 
$query_run = q($query);

if(isset($_REQUEST['submit'])){
		$query = "insert into fs_audition_msg(audition_id, sender_id,receiver_id,message,send_time)values('".$_REQUEST['audition_id']."',
		'".$_SESSION['fab_id']."','".$_REQUEST['talent_id']."','".$_REQUEST['message']."',
		'".date('Y-m-d h:i:s')."')";
		$insert = q($query);
		if($insert){
...
<?include('top.php');?> 	
			<div>
                    	...
                    	<tr>
                        	<td class="message_listing">
                            	<ul>
									...
									$qls_comp = "select * from fs_mamber where fld_id = '".$title_res['talent_id']."'";
										...
										$qls_fatch_comp = f($qls_run_comp);
										   ...
										   $title_query11 = q("SELECT * FROM  fs_message WHERE (sender_id='".$_SESSION['fab_id']."' OR sender_id='".$job_result['talent_id']."') AND (receiver_id='".$_SESSION['fab_id']."' OR receiver_id='".$job_result['talent_id']."') AND job_id = '".$job_result['job_id']."' ORDER BY fld_id desc limit 1 ");
                                    ...
                                    $title_query12 = q("SELECT * FROM  fs_message WHERE (sender_id='".$_SESSION['fab_id']."' OR sender_id='".$job_result['talent_id']."') AND (receiver_id='".$_SESSION['fab_id']."' OR receiver_id='".$job_result['talent_id']."') AND (job_id = '".$job_result['job_id']."') AND talent_status=1 ");
                                     $title_res12 = nr($title_query12);

                                    ?>	

									<li>
                                    <a href="message_company_page.php?job_id=<?=$job_result['job_id']?>&talent_id=<?=$job_result['talent_id']?>">
                               ...
                               <?php
									...
									$qls_comp = "select * from fs_mamber where fld_id = '".$title_res1['talent_id']."'";
										$qls_run_comp = q($qls_comp);
										$qls_fatch_comp = f($qls_run_comp);
									...
									(sender_id='".$_SESSION['fab_id']."' OR sender_id='".$job_result1['talent_id']."') AND (receiver_id='".$_SESSION['fab_id']."' OR receiver_id='".$job_result1['talent_id']."') AND audition_id='".$job_result1['audation_id']."' ORDER BY fld_id desc limit 1 ");
									...
									(sender_id='".$_SESSION['fab_id']."' OR sender_id='".$job_result1['talent_id']."') AND (receiver_id='".$_SESSION['fab_id']."' OR receiver_id='".$job_result1['talent_id']."') AND (audition_id='".$job_result1['audation_id']."') AND talent_status=1");
									...
									<li>
                                    <a <?if($job_result1['audation_id']==$_REQUEST['audition_id'] AND $job_result1['talent_id']==$_REQUEST['talent_id']){?>class="active"<?}?> href="message_company_audition_page.php?audition_id=<?=$job_result1['audation_id']?>&talent_id=<?=$job_result1['talent_id']?>">
									<?if($qls_fatch_comp['profile_crop_image']!=''){?>
									<img src="<?=$qls_fatch_comp['profile_crop_image']?>" width="50" height="50" />
									<?}else{?>
									...
									?>
                                    	<div class="message_text">
                                        <h1><?=ucfirst(substr($title_res1['title'],0,30))?>... <span><?=$date_time?></span></h1>
                    ...
                    <table class="center_body" width="100%" cellspacing="1" cellpadding="1" border="0" align="right" style="margin-top:0px;float:right; background:#fff;">
                    	<tr>
                        	<td>
							    ...
							    <?
							  $co_name = "select * from fs_mamber where fld_id = '".$_REQUEST['talent_id']."'";
							  $co_comp = q($co_name);
							  $coc_comp = f($co_comp);
							  ...
							  ?>
								...
								<? 
                                   ...
                                   $message_list_view = q("SELECT * FROM fs_audition_msg WHERE
									(sender_id='".$_SESSION['fab_id']."' OR sender_id='".$_REQUEST['talent_id']."') AND (receiver_id='".$_SESSION['fab_id']."' OR receiver_id='".$_REQUEST['talent_id']."') AND audition_id='".$_REQUEST['audition_id']."' order by fld_id DESC");
									...
									$count_record = nr($message_list_view);
				...
				<form id="upload_form" name="upload_form" autocomplete="off" action="" onSubmit="return validate();" method="POST">
                   ...
                   <input type="hidden" id="audition_id" value="<?=$_REQUEST['audition_id']?>">
                   <input type="hidden" id="talent_id" name="talent_id" value="<?=$_REQUEST['talent_id']?>">
