...
if($_FILES['pic']['name'] != ""){	//UPLOAD LOGO
	...
	//LARGE PIC
	$uploaddir = 'admin/portfolio_talent_audio/';
	$input_name = 'pic';
	...
	if($file['error'] == UPLOAD_ERR_OK){
		if(isAllowedExtension($file['name'])){
			if(move_uploaded_file($_FILES[$input_name]['tmp_name'], $uploaddir.$uploadfile)){
				...
				$sql = "INSERT INTO fs_portfolio_talent_audio (ex_id, title, large_pic,  added_on) VALUES ('".$id."', '".$_REQUEST['title']."', '".$uploadfile."',  '".date('Y-m-d h:i:s')."')";


				$sql = q($sql);
				if($sql){
					$msg1 = ' Audio has been Added Successfully.';
				}
			}else{
				$msg1 = ' Audio has not been Uploaded Successfully.';
			}
										}else{
...
function validate(){
	...
	if(document.upload_form.title.value == "" ){
		 errText = "- Please Enter the Audio Title.";
		 alert(errText);
		 errorflag = true;
		...
		if(document.upload_form.pic.value == "" ){
		 errText = "- Please  Browse the Portfolio Audio.";
		 alert(errText);
		 errorflag = true;
...
function delete_pub(id,id1){
	...
	window.location.href='delete_talent_audio_front.php?id='+id+'&id1='+id1;
}
</script>
...
<tr>
  ...
  <tr>
    <td colspan="2" align="left" class="text_heading">Add Portfolio Audio :   </td>
  </tr>
  <tr>
		...
		<table cellpadding="2" cellspacing="2">
		   <tr>
			  ...
			  <tr>
			   ...
			   <tr>
					<td width="150px"><span class="style1">*</span>&nbsp;<span class="text_heading">Browse  Audio</span> </td>
			  ...
			  <tr>
			   ...
			   <tr>
				  <td>&nbsp;</td>
					<td>
					  <input type="submit" value="Add Audio" class="button" name="Submit">
				   </td>
				</tr>
		</table>
		<form>
  ...
  <tr>
   ...
   <tr>
    <td colspan="2" align="left" class="text_heading">Portfolio Audio List</td>
  </tr>
   <tr>
    <td colspan="2" height="10" align="center"></td>
  </tr>
    ...
    <td colspan="2" height="8" width="100%">
		<table cellpadding="1" cellspacing="1" width="100%">
		   <tr style="background-color: #669966">
				...
				<td style="font-family: arial; font-size: 15px; font-weight: bold; color: #ffffff" height="25px">Audio</td>
				
				<td style="font-family: arial; font-size: 15px; font-weight: bold; color: #ffffff" height="25px">Delete</td>

			  </tr> 
			  <?
				$sqlg = "Select * from fs_portfolio_talent_audio Where ex_id = '".$id."' order by fld_id DESC";
				$resg = q($sqlg);
				$countg = nr($resg);
				if($countg){
					while($rowg = f($resg)){
			  ...
			  <tr>
					...
					<td height="25px"><img src="admin/portfolio_talent_audio/<?=$rowg['large_pic']?>" width="100px"></td>
			...
			<?		}
				...
				<tr>
					<td height="25px" colspan=4  style="font-family: arial; font-size: 15px; font-weight: bold; color: #ff0000">No  Audio Available.</td>
			  </tr>	
			<?}?>
		</table>						
	</td>
