                  ...
                  if($_REQUEST['video_link']!=''){
								 ...
								 $video = explode(",",$video_link);
									foreach ($video as &$link) {
									   $Insert = "insert into fs_talent_project_video_link(talent_id,project_id,video_link,added_on)values('".$id."',0,'".$link."','".date('Y-m-d h:i:s')."') ";
	...
	<div id="content_wrap" style="z-index:100;">	
		...
		<div style="float:left; clear:both;">
			<div style="float:left; width:170px; clear:both;">
				<?include('include_talent_skill1.php');?>
			</div>
			...
			<div style="width:820px; float:left; margin-bottom:25px;">
				<?
				if($_REQUEST['skill']==''){
					$query_pro = "SELECT * FROM fs_talent_project where talent_id='".$id."' order by project_title asc";
				}else{
					$query_pro = "SELECT * FROM fs_talent_project where talent_id='".$id."' and skill LIKE '%".$_REQUEST['skill']."%' order by project_title asc";
				}
				$run_pro = q($query_pro);
...
<!-----end add project here------------>
            ...
            <ul>
				<?while($fatch_pro = f($run_pro)){ 
				  ...
				  $png = ".png";
				  $gif = ".gif";
				  $select_image = "select * from fs_project_portfolio where talent_id='".$id."' AND project_id='".$fatch_pro['fld_id']."' AND (portfolio_data LIKE '%".$jpg."%' OR portfolio_data LIKE '%".$png."%' OR portfolio_data LIKE '%".$gif."%') limit 1";
			...
			if($counter==1){
            ?>
			<li><a href="talent_project_details.php?id=<?=$fatch_pro['fld_id']?>&talent_id=<?=$_REQUEST['talent_id']?>">
