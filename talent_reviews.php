...
$talent_id=$_REQUEST['talent_id'];
   ...
   if(isset($_REQUEST['submit'])){
	...
	$sql_duplicate = "SELECT * FROM fs_company_reviews WHERE company_id = '".$company_id."' AND job_id = '".$job_id."' AND talent_id = '".$talent_id."' ";
	$res_duplicate = q($sql_duplicate);
	$count = nr($res_duplicate);
	if($count == 0){
		
		$query = "INSERT INTO fs_company_reviews(company_id, job_id,talent_id,terms1,terms2,terms3,terms4,terms5,added_on)
		...
		$insert = q($query);
		if($insert){
			 $message= "You Are Finish Job & Submit Reviews Successfully";
		}else{
		   $message= "You Are Not Finish Job & Submit Reviews Successfully";
		}
   }else{
		$message= "You Are Already Finish & Submit Reviews For This Job";
   }
	   if($_REQUEST['st']=='accept'){
    $sql_up="update fs_applied_job set company_request=3,status=4 WHERE company_id = '".$company_id."' AND job_id = '".$job_id."' AND talent_id = '".$talent_id."' ";
   $res_up=q($sql_up);
	   }else{
